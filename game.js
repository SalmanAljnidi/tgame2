...
(() => {
  ...
  const sfx = {
    ...
    // منع قائمة الضغط المطوّل/الزر الأيمن داخل اللعبة
...
btnSound.addEventListener('click', () => {
  ...
  // iOS: نضيف TouchEvents على الكانفس فقط، وبشكل آمن (لا يؤثر على شاشة إدخال الاسم)
  function gameIsActive(){
    return startOverlay.classList.contains('hidden');
  }
  canvas.addEventListener('touchstart', (e)=> {
    if (!gameIsActive()) return;      // لا تمنع تفاعل شاشة البداية
    e.preventDefault();
    startHold();
  }, {passive:false});

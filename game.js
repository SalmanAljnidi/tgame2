...
(() => {
  ...
  function showQuiz(onResolve){
    ...
    reprBox.innerHTML = '';
    if (isAdd){
      const g1 = buildGroup('', a, 0);
      const opB = opBadge('+');
      const g2 = buildGroup('', b, 0);
      reprBox.appendChild(g1);
      reprBox.appendChild(opB);
      reprBox.appendChild(g2);
    } else {
      const g1 = buildGroup('', a, b);
      const opB = opBadge('−');
      reprBox.appendChild(g1);
      reprBox.appendChild(opB);
    }
  ...
  function buildGroup(title, count, removedCount){
    // title يمكن يكون فارغاً إذا ما نحتاج تكرار الأرقام
